...
public class Bat6 : MonoBehaviour
{
    ...
    public int value_to_upgrade = 350; //valeur pour ameliorer
    public int people_need_to_upgrade = 0;
    public int number_of_employee = 0;

    [SerializeField] List<Bat1_Upgrades> Bat6_Upgrades; //usage de la classe Bat1_Upgrade pour modif l'affichage/les vlaeurs

    public TextMeshProUGUI bat6_price_txt; //prix affiché sur bouton
    public TextMeshProUGUI bat6_name; //nom affiché ingame
    public TextMeshProUGUI bat6_update_description; //description affiché sur bouton
    public Image bat6_update_sprite; //image affiché ingame
    ...
    private void Start()
    {
        Bat4Update(Bat6_Upgrades[level]); //reference le bat au start avec son niveau 0
    ...
    {
        //tuto launch
        if (bat4.level >= 10 && dialog_done == false)
        {
            ...
            //bloqueur
            mairieniv10.SetActive(false);
        }

        maingame.CheckCanBuy(bat6_price_txt, R_and_P.brick_number, value_to_upgrade); //vérifier si achetable pour afficher texte couleur
    ...
    public void UpgradeDone()
    {
        ...
        Bat4Update(Bat6_Upgrades[level]); //recupere les valeurs de l'update
        ...
        if (level == 1) //upgrade du niv1
        {
            ...
            //caillou explose
            ShowKayou(Bat6_Pos);
            R_and_P.people_augmentation = (int)(R_and_P.people_augmentation + 2);
            R_and_P.brick_augmentation = (int)(R_and_P.brick_augmentation + 2);
        }
        if (level >= 2)
        {
            ShowKayou(Bat6_Pos);
    ...
    public void Hit(Transform Hit_Pos)
    {
        ...
        else if (isUpgrading == true)
        {
            ...
            maingame.ShowStar(Bat6_Pos,-7f,2f);
            //upgrade
            lerp = 0;
            ...
            go.transform.position = new Vector2(go.transform.position.x , go.transform.position.y +0.2f);
            Visual.transform.DOComplete(); //complete l'animation précédente pour éviter bug
    ...
    public void Bat4Update(Bat1_Upgrades _bat2upgrade) //modifier visuelement le bat
    {
        ...
        bat6_price_txt.text = (value_to_upgrade).ToString(); //change le prix
        bat6_name.text = _bat2upgrade.Name; //change le nom
        bat6_update_description.text = _bat2upgrade.Description; //change la description de l'update
        bat6_update_sprite.sprite = _bat2upgrade.SpriteUpdate; //change le sprite de l'icone dans l'update
        ...
        if (Bat6_Upgrades.Count <= level +1)
        {
